<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="组织编号" prop="zuzhibianhao">
          <el-input v-model="ruleForm.zuzhibianhao" 
              placeholder="组织编号" clearable  :readonly="ro.zuzhibianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="组织编号" prop="zuzhibianhao">
              <el-input v-model="ruleForm.zuzhibianhao" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zuzhitupian" label="组织图片" prop="zuzhitupian">
          <file-upload
          tip="点击上传组织图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.zuzhitupian" label="组织图片" prop="zuzhitupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zuzhitupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="组织宣言" prop="zuzhixuanyan">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="组织宣言"
                  v-model="ruleForm.zuzhixuanyan" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zuzhixuanyan" label="组织宣言" prop="zuzhixuanyan">
                    <span>{{ruleForm.zuzhixuanyan}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zuzhimingcheng = true;
          ...
          if(o=='zuzhixuanyan'){
            this.ruleForm.zuzhixuanyan = obj[o];
	    this.ro.zuzhixuanyan = true;
            continue;
          }
          if(o=='fuzeren'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.zuzhibianhao!=''&&json.zuzhibianhao){
                this.ruleForm.zuzhibianhao = json.zuzhibianhao
	    		this.ro.zuzhibianhao = true;
		}
	...
	this.ruleForm.zuzhijieshao = this.ruleForm.zuzhijieshao.replace(reg,'../../../ssmj85gg/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zuzhixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zuzhixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
